...
public class MixinServerGamePacketListenerImpl {
  ...
  private boolean LootrAllowInteractSpawnProtection (ServerLevel instance, Entity player, BlockPos blockPos, Operation<Boolean> original) {
    // TODO: This has the potential to break if `mayInteract` is ever extended to do more than just spawn protection checks.
    if (LootrAPI.shouldBypassSpawnProtection() && player instanceof Player player2) {
      if (instance.getBlockState(blockPos).is(LootrTags.Blocks.INTERACT_WHITELIST_BLOCKS) && instance.getServer()
          .isUnderSpawnProtection(instance, blockPos, player2)) {
