...
public class LootrAdvancementGenerator implements AdvancementProvider.AdvancementGenerator {
  ...
  public void generate(HolderLookup.Provider arg, Consumer<AdvancementHolder> consumer, ExistingFileHelper existingFileHelper) {
    ...
    // all gravels
    AdvancementHolder brush = Advancement.Builder.advancement().parent(lootrRoot).display(LootrRegistry.getSuspiciousGravel(), Component.translatable("lootr.advancements.all_gravel.title"), Component.translatable("lootr.advancements.all_gravel.description"), null, AdvancementType.TASK, true, true, false).addCriterion("brushed_gravel", ContainerTrigger.looted(LootrRegistry.getGravelTrigger())).addCriterion("brushed_sand", ContainerTrigger.looted(LootrRegistry.getSandTrigger())).save(consumer, LootrAPI.rl("all_gravel"), existingFileHelper);
    // 10loot
