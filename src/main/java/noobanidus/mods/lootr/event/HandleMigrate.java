    ...
    if (!migrations.isEmpty()) {
      ...
      for (Map.Entry<Path, Path> migrationEntry : migrations.entrySet()) {
        try {
          ...
          LootrAPI.LOG.info("Unable to migrate from '" + migrationEntry.getKey() + "' to '" + migrationEntry.getValue() + "' as destination already exists! Deleting extraneous file.");
          try {
            Files.delete(migrationEntry.getKey());
          } catch (IOException ex) {
            LootrAPI.LOG.error("Unable to migrate from '" + migrationEntry.getKey() + "' to '" + migrationEntry.getValue() + "' as destination already exists! Unable to delete extraneous file. Please manually delete.", e);
          }
        } catch (IOException e) {
